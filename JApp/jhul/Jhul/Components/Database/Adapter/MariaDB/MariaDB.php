...
class MariaDB
{
	...
	public function _delete( $item )
	{
		...
		(
			$this->makeStatement( 'delete'  )->setTable( $item->_p()->tableName() )->where( $item->_p()->keyName(),  $item->key() )
		);
	...
	public function counter( $prototype )
	{
		return new Count( $prototype );
	}
	...
	public function insert( $item )
	{
		...
		$statement = $this->executeStatement
		(
			$this->makeStatement( 'insert'  )->setTable( $item->_p()->tableName() )->setData( $item->_mData()->_get() )
		);
	...
	public function update( $item )
	{
		...
		$statement = $this->executeStatement
		(
			$this->makeStatement( 'update'  )->setTable( $item->_p()->tableName() )
			->setData( $item->_mData()->_get() )->where( $item->_p()->keyName(),  $item->key() )
		);
