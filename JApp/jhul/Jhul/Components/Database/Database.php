...
class Database
{
	...
	public function getStore( $storeClass )
	{
		if(empty($this->_stores[$storeClass]))
		{
			...
			{
				throw new \Exception( 'DAO Manager Class '.$storeClass.' Not Found', 1);
			}
	...
	public function getParamBag( $paramBagClass )
	{
		return $this->_paramBags[$paramBagClass] ;
	}

	public function hasParamBag( $paramBagClass )
	{
		return isset($this->_paramBags[$paramBagClass]) ;
	}

	public function createParamBag( $paramBagClass, $item )
	{
		$this->_paramBags[$paramBagClass] = new $paramBagClass($item);
	} 
}
