...
abstract class _Class
{
	...
	public function reloadFieldManager()
	{
		...
		$this->_mFile = new FieldManager( $this->loadFiles( $this->files() ) ) ;

	}

	private function loadFields( $requireFields )
	{
		$definitionMap = $this->J()->fx()->loadConfigFile( $this->_d()->path().'/_config/_form_fields');
	...
	private function loadFiles( $requireFields )
	{
		$definitionMap = $this->J()->fx()->loadConfigFile( $this->_d()->path().'/_config/_form_fields');
	...
	public function getValue( $field, $required = TRUE )
	{
		if( $this->submission()->has($field) )
		{
