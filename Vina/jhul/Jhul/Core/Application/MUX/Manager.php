<?php namespace Jhul\Core\Application\MUX;
...
class Manager
{
	...
	public function run( $handler, $params = [] )
	{
		...
		if( !$this->mPath()->hasNext() || $handler->canHandleNextNode() )
		{
			...
			{
				$context = $handler->resolveContext();

				if( !empty($context) )
				{
					$handler->_m()->context($context)->handle();
				}
				else
				{
					$handler->handle( $this->_data ) ;
				}
			}
		}
	}
	...
	public function resolveClass( $handler )
	{
		if( !strrpos( $handler, '\\' ) )
		{
			$handler = $this->app()->mapper()->getMUXClass($handler);
		}
